...
app.get('/', (req, res) => {
    res.status(200).send('<h1>Welcome to the Distance Game API!</h1><p>The server is running correctly.</p>');
});
...
app.post('/api/submit', async (req, res) => {
    ...
    if (playerId === undefined || distance === undefined) {
        return res.status(400).json({ error: 'Missing playerId or distance in request body.' });
    }
    ...
    if (!gameState) {
        return res.status(404).json({ error: 'No active game found for this player. Please start a new game.' });
    }
    ...
    if (!playerSocket) {
        return res.status(500).json({ error: 'Internal server error: Player socket not found.' });
    }
    ...
    if (gameState.lives <= 0) {
        console.log(`Game over for player ${playerId}. Final score: ${gameState.score}`);
        try {
